...
\item \texttt{veto}: If set to \texttt{.true.} then impose exclusivity veto in ion--ion production due to ion electromagnetic dissociation, as discussed in 2604.19879.
This is achieved by parameterising $p_{vb}(E_\gamma)$, where $E_\gamma$ is the photon energy in the dissociating ion rest frame, according to
\begin{equation}
  p_{vb}(E_\gamma) = {\rm Max}\left[{\rm ERF} \left[ \frac{E_\gamma - {\rm veto\_ecent}}{{\rm veto\_eden}} \right],0\right]
\end{equation}
see Fig. 1 (b) of this reference.
\item \texttt{veto\_ecent}: Parameter of ERF function as defined above.
\item \texttt{veto\_eden}: Parameter of ERF function as defined above.
\item \texttt{rshad}: Nuclear shadowing factor, $S_g$, for high energy $\gamma A$ cross section.
\item \texttt{wrho}: if set to \texttt{.true.} then includes probability of coincident $\rho^0$ production in PbPb collisions, as described in arXiv:2506.03264.
\item \texttt{yrho}: maximum value of $|y_{\rho^0}|$, if \texttt{wrho} is \texttt{.true.}.
\item \texttt{accrho}: additional correction factor to account for e.g. $\rho^0\to \pi^+\pi^-$ acceptance, non-resonant dipions and different beams (input assume Pb beams). Simply multiplies $\gamma A \to \rho^0 A$ cross section.
 Default normalization is as described in arXiv:2506.03264.
...
%In particular, for purely elastic production (i.e. with \texttt{diff} set to \texttt{el}) the following flags should be set:
%\\
%\\
...
55 & $W^+(\to \nu_l/u(8)+l^+/\overline{d}(9))\,+\, W^-(\to \overline{\nu}_l/\overline{u}(10)+l^-/d(11))$ &  $pA$-el  $AA$-el  $ee$-el  $pp$-dd  $pp$-sda  $pp$-el  \\
